...
/**
 * Pure game actions hook - contains only business logic, no UI
 * This can be used by both mobile and desktop UI components
 *
 * This is a simplified version of the original useGame hook for use in the new modular architecture
 */
export function useGameActions() {
  ...
  const playCard = useCallback(async (position: Position) => {
    ...
    // Update game object for immediate turn indicator update
    const updatedGame = {
      ...game,
    ...
    try {
      ...
      const { error: rpcError } = await supabase.rpc('play_card_optimized', {
        p_game_id: gameId,
